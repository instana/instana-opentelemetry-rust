...
fn test_instana_exporter_options_with_endpoint() {
    ...
    let options = InstanaExporterOptions::with_endpoint(endpoint).unwrap();
...
fn test_instana_exporter_options_with_invalid_endpoint() {
    ...
    let options = InstanaExporterOptions::with_endpoint(invalid_endpoint);
...
async fn test_instana_exporter_export_success() {
    ...
    let options = InstanaExporterOptions::with_endpoint(&format!("{}/test-path", mock_server.uri())).unwrap();

    let client: Arc<dyn HttpClient> =
        Arc::new(reqwest::Client::builder().build().unwrap_or_default());

    let exporter = InstanaExporter::new(client, options, get_resource());
...
fn test_instana_exporter_shutdown() {
    ...
    let options = InstanaExporterOptions::default();
    let mut exporter = InstanaExporter::new(client, options, get_resource());
...
fn test_instana_exporter_set_resource() {
    ...
    let mut exporter = InstanaExporter::new(client, options, base_resource.clone());
